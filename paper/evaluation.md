...
- Integration artifacts for Web/JavaScript, TypeScript, Dart/Flutter, C FFI, CLI, CI/action, and a browser reference-monitor demo.
...
- Web/JavaScript、TypeScript、Dart/Flutter、C FFI、CLI、CI/action、browser reference-monitor demoの組み込み形態を示す。
...
この評価は、危険fixtureの検出、安全fixtureでの非検出、危険mutationによるscore上昇、高リスクruntime claimのdeny/rewrite、policy改ざん拒否、demo appにおける直接claim描画の不在、fuzzされた高リスクrequestの非allow、no-network coreにnetwork API呼び出しがないことをすべて満たす場合にのみpassする。
